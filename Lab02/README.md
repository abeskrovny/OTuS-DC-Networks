...
> В Arista EOS контекст интерфейса является более высоким, чем контекст процесса динамического роутинга, что подволяет гранулярно отключить BFD на интерфейзе, занятом в процессе динамической маршрутизации, в которой глобально включен BFD командой `no ipv6 ospf bfd`. Привести к поведению по-умолчанию можно префиксом  `default`.
...
- защите (Control Plane Protection).
