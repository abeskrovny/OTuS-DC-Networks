...
- [Результирующая конфигурация с предыдущей стадии](#результирующая-конфигурация-с-предыдущей-стадии)
...
- [Конфигурирование Underlay сети с использованием OSPF](#конфигурирование-underlay-сети-с-использованием-ospf)
    - [IPv4 (OSPFv2)](#ipv4-ospfv2)
    - [IPv6 (OSPFv3)](#ipv6-ospfv3)
- [Настройка BFD](#настройка-bfd)
...
Чтобы корректно и без потерь трафика вывести из работы (направить трафик в обход) коммутатор Arista в Underlay-сети на базе OSPF, нужно задействовать механизм Max-Metric (Overload). Эта функция плавно «выписывает» коммутатор из активных путей передачи транзитного трафика, заставляя соседей пересчитать маршруты через резервные узлы.

Делается это в контексте экземпляра процесса маршрутизации через установку максимальной стоимости (metric) анонсируемых маршрутов в Router-LSA. Это заставит соседей считать этот узел наименее предпочтительным:
```ssh
router ospf 4
    max-metric router-lsa
```

Ждем несколько секунд, пока соседи получат обновленные LSA и перестроят таблицы маршрутизации (SPF), после убеждаемся, что через коммутатор перестал идти транзитный трафик:
```ssh
ip ospf neighbor
show ip route ospf
```

После того как трафик полностью ушел с устройства, можно выполнять запланированные работы (выключение, обновление EOS и т.д.).

Возврат осуществляется вводом команды `no max-metric router-lsa` в контексте экземплара процесса маршрутизации.