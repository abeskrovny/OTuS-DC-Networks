...
На Underlay-уровне (базовой физической сети) лучше использовать протоколы Link-State (OSPF, IS-IS), а не дистанционно-векторные (RIP, EIGRP), потому что они обеспечивают минимальное время сходимости, отсутствие топологических петель и точную информацию о карте сети для overlay-технологий (EVPN-VXLAN, SD-WAN).
