...
1. Соберете топологию CLOS, как в приложенной схеме
...
!!!
 ...
 В инфраструктурах EVPN/VXLAN инкапсуляция добавляет к оригинальному пакету минимум 50 байт оверхеда (VXLAN + UDP + IP + Ethernet). Чтобы конечные серверы могли свободно передавать свои Jumbo Frames (стандартные 9000 байт), магистраль должна иметь запас. Выставление 9214 на линках между Leaf и Spine полностью убирает проблему фрагментации.

Надо ли настраивать MTU для Loopback-интерфейсов?

Нет, для интерфейсов Loopback изменять MTU не нужно. Оставляйте стандартное дефолтное значение (обычно 65535 или 1500 в зависимости от операционной системы, в Arista EOS по умолчанию L3 MTU равен 1500).

Почему это не требуется: Loopback — это виртуальный интерфейс внутри процессора маршрутизатора. Он не генерирует и не передает транзитные физические кадры. Изменение MTU на Loopback никак не влияет на прохождение инкапсулированного VXLAN-трафика через коммутатор.

Влияние на IS-IS: Когда вы активируете IS-IS на Loopback (командой isis enable), интерфейс просто анонсирует свой IP-префикс в базу данных (LSP). Он не отправляет Hello-пакеты во внешний мир через петлю, поэтому размер MTU здесь не имеет значения для установления сессий.


> Учитывая размер накладного заголовка VxLAN, используемого для построения наложенной сети, можно на этом этапе включить использование длинных кадров (Jumbo-frame) на вовлеченных интерфейсах.
